...
- Every behavior is pinned by CPython output, not by reasoning
  - Regenerate `tests/fixtures/values.json` with `tests/fixtures/generate.py`; add a corpus row before changing behavior
  - Divergences go in `KNOWN` in `tests/fixtures.rs` with a reason; an entry that starts matching fails until deleted
  - Regenerate `src/repr/nonprintable.rs` with `scripts/generate_nonprintable.py` when the target Python's Unicode version changes
  - `verify_rust_pickles.py` checks CPython reads Rust pickles; CI does not run Python
- Decoders reject nesting beyond `MAX_DEPTH`; `benches/formats.rs` guards linear cost by depth
