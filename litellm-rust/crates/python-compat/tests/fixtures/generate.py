"""Regenerate values.json: what CPython produces for each value in CORPUS.

    python tests/fixtures/generate.py > tests/fixtures/values.json
...
def evaluate(source):
    with warnings.catch_warnings():
        ...
        try:
            return {"source": source, "repr": repr(ast.literal_eval(source))}
    ...
    json.dump(
        {
            ...
            "rows": [row(source) for source in CORPUS],
            "sources": [evaluate(source) for source in SOURCES],
        },
        sys.stdout,
        indent=2,
        ensure_ascii=True,
