//! Replays `tests/fixtures/values.json`, which CPython generated with `generate.py`.
...
fn fixtures() -> Fixtures {
    serde_json::from_str(include_str!("fixtures/values.json"))
...
impl Mismatches {
    ...
    fn check(&mut self, row: &Row, what: &str, expected: &str, actual: &str) {
        self.check_source(&row.source, what, expected, actual);
    }

    fn check_source(&mut self, source: &str, what: &str, expected: &str, actual: &str) {
        let known = Self::known(source, what);
...
#[test]
fn literal_rows_match_python_repr_str_bool_and_json() {
    let mut mismatches = Mismatches::default();
    for row in rows().iter().filter(|row| row.literal) {
...
#[test]
fn literal_eval_matches_python_on_source_texts() {
    let mut mismatches = Mismatches::default();
    for case in fixtures().sources {
        ...
        mismatches.check_source(&case.source, "literal_eval source", &expected, &actual);
    }
    mismatches.finish();
}

#[test]
fn pickle_loads_matches_python_at_every_protocol() {
    let mut mismatches = Mismatches::default();
    for row in rows() {
...
#[test]
fn values_reached_only_through_pickle_match_python() {
    let mut mismatches = Mismatches::default();
    for row in rows()
        .iter()
...
#[test]
fn pickle_dumps_round_trips_every_plain_literal() {
    ...
    for row in rows()
        ...
        .filter(|row| row.literal && row.plain && row.view.as_deref() == Some(&row.repr))
    {
        let value = literal_eval(&row.repr).expect("literal row parses");
...
#[test]
fn to_json_matches_python_json_round_trip() {
    let mut mismatches = Mismatches::default();
    for row in rows().iter().filter(|row| row.literal) {
